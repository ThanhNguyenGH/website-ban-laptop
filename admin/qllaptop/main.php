<?php include("../inc/top.php"); ?>

<h3>Quản lý Laptop</h3> 
<br>
<a href="index.php?action=them" class="btn btn-info">
	<i class="align-middle" data-feather="plus-circle"></i> 
	Thêm laptop
</a>
<br> <br> 
<table class="table table-hover">
	<tr>
		<th>Tên laptop</th>
		<th>Hãng</th>
		<th>Giá bán</th>
		<th>Số lượng</th>
		<th>Hình ảnh</th>		
		<th>Sửa</th>
		<th>Xóa</th>
	</tr>
	<?php
	foreach($laptops as $m):
	?>
	<tr>
		<td>
			<a href="index.php?action=chitiet&id=<?php echo $m["id"]; ?>">
			<?php echo $m["tenlaptop"]; ?>
			</a>	
		</td>
		<td><?php echo $m["tenhang"] ?? ""; ?></td>
		<td><?php echo number_format($m["giaban"]); ?>đ</td>
		<td><?php echo $m["soluongton"]; ?></td>
		<td>
			<a href="index.php?action=chitiet&id=<?php echo $m["id"]; ?>">
			<?php 
			$hinhanh = isset($m["hinhanh"]) && !empty($m["hinhanh"]) ? $m["hinhanh"] : "images/laptops/default.jpg";
			?>
			<img src="../../<?php echo $hinhanh; ?>" width="80" class="img-thumbnail"></a>
		</td>
		<td><a class="btn btn-warning" href="index.php?action=sua&id=<?php echo $m["id"]; ?>"><i class="align-middle" data-feather="edit"></a></td>
		<td><a class="btn btn-danger" href="index.php?action=xoa&id=<?php echo $m["id"]; ?>"><i class="align-middle" data-feather="trash-2"></a></td>
	</tr>
	<?php
	endforeach;
	?>
</table>

<?php include("../inc/bottom.php"); ?>
